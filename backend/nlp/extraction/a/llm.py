...
MODEL = os.environ.get("FINAGENT_LLM_MODEL", "qwen2.5:3b")
...
def llm_hazir() -> bool:
    """Ollama ayakta ve model yüklü mü? (pipeline'ın zarif geri düşüşü için)"""
    try:
    ...
    try:
        cevap = requests.post(
            ...
            json={
                ...
                # num_ctx=2048 uzun kampanya metinlerinde prompt'un kuyruğunu
                # (METİN bölümünü) kırpıyordu — model metni tam göremeden
                # cevap veriyordu. 4096, 3B modelde CPU/RAM'e rahat sığar
                # (~0.5 GB ek KV cache).
